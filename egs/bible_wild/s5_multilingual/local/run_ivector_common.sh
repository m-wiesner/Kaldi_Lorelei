...
if [ $stage -le 1 ]; then
  ...
  steps/make_mfcc_pitch.sh --cmd "$train_cmd" --nj 32 data/${train_set}_sp || exit 1;
...
if [ $stage -le 2 ]; then
  ...
  steps/align_fmllr.sh --nj 20 --cmd "$train_cmd" \
    data/${train_set}_sp ${langdir} $gmm_dir $ali_dir || exit 1
fi
...
if [ $stage -le 3 ]; then
  ...
  if [[ $(hostname -f) == *.clsp.jhu.edu ]] && [ ! -d $mfccdir/storage ]; then
    utils/create_split_dir.pl /export/b{10,12}/$USER/kaldi-data/mfcc/mini_librispeech-$(date +'%m_%d_%H_%M')/s5/$mfccdir/storage $mfccdir/storage
  fi
...
if [ $stage -le 6 ]; then
  ...
  if [[ $(hostname -f) == *.clsp.jhu.edu ]] && [ ! -d $ivectordir/storage ]; then
    utils/create_split_dir.pl /export/b{10,12}/$USER/kaldi-data/ivectors/mini_librispeech-$(date +'%m_%d_%H_%M')/s5/$ivectordir/storage $ivectordir/storage
  fi
